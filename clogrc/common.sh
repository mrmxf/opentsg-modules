...
# --- clog functions ----------------------------------------------------------
[ -f clogrc/inc.sh ] && source core/inc.sh
...
function fPrompt() {
	...
	local DFT=$(echo "$OPT" | grep -E -o "[A-Z]") # default response is in CAPS
	...
	while true; do
		if [[ -n "$isZSH" ]]; then
			read -t -r "$SEC" -k key # zsh read char in time
			err=$?
		else
			read -t -r "$SEC" -n 1 key # bash read char
			err=$?
		fi
