...
function getRemoteTag() {
	...
	local TAG=$(git ls-remote --tags "$URL" v\* 2>/dev/null | tail -1 | sed -r 's/.*(v[\.0-9]*).*/\1/')
	if ! mycmd; then
		((OOPS++)) && return $OOPS # unknown error on stdout
	fi
	if [ -z "$2" ]; then
...
# --- tag handling ------------------------------------------------------------
vRnode=$(getRemoteTag opentsg-node ref)
OOPS=$?
vREF="$vRnode"
...
vRcore=$(getRemoteTag opentsg-core)
OOPS=$?
vRio=$(getRemoteTag opentsg-io)
OOPS=$?
vRlab=$(getRemoteTag opentsg-lab)
OOPS=$?
vRmhl=$(getRemoteTag opentsg-mhl)
OOPS=$?
vRwidgets=$(getRemoteTag opentsg-widgets)
OOPS=$?
